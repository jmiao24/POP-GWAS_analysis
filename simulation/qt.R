...
for (h2_X_Y in h2_X_Y_vec){
  ...
  result <- foreach(
    ...
    z <- scale(g) * sqrt(h2_X_z) + y * sqrt(1 - var_res - h2_X_z) + grt_err(n.total, 1) * sqrt(var_res)
  ...
  fwrite(as.data.frame(result), paste0("./results/tmp/margin_eff.qt.", mar_effect, ".txt.gz"),
